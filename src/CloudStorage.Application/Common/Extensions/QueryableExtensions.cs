...
internal static class QueryableExtensions
{
    ...
    {
        return query.AnyAsync(x => x.Id == id && x.IsFolder && !x.IsDeleted, cancellationToken);
    }

    internal static Task<Resource?> FindFileByIdAsync(this IQueryable<Resource> query, Guid id,
        CancellationToken cancellationToken = default)
    {
        return query.FirstOrDefaultAsync(x => x.Id == id && !x.IsFolder && !x.IsDeleted, cancellationToken);
    }
}